...
bool Sample::Init() 
{
	...
	for (auto fbx : m_FBXList)
	{
		...
		{
			K_FbxObject* pObj = fbx->m_pDrawObjList[iObj];
			std::wstring  szLoad = szDefaultDir + pObj->m_vTextureNameList[0];

			pObj->Create(m_pd3dDevice, m_pImmediateContext,
				 szLoad, shaderfilename);
		}
	}
...
bool Sample::Render()
{
	...
	for (int iModel = 0; iModel < m_FBXList.size(); iModel++)
	{
		for (int iObj = 0; iObj < m_FBXList[iModel]->m_pDrawObjList.size(); iObj++)
		{
			...
			pObj->m_fAnimSpeed = 1.1f;

			pObj->m_fAnimeFrame = pObj->m_fAnimeFrame + (g_fSecondPerFrame * pObj->m_AnimeScene.fFrameSpeed * pObj->m_fAnimSpeed * pObj->m_fAnimInverse);
			if (pObj->m_fAnimeFrame > pObj->m_AnimeScene.iEndFrame ||
				pObj->m_fAnimeFrame < pObj->m_AnimeScene.iStartFrame)
			{
				pObj->m_fAnimeFrame = min(pObj->m_fAnimeFrame, pObj->m_AnimeScene.iEndFrame);
				pObj->m_fAnimeFrame = max(pObj->m_fAnimeFrame, pObj->m_AnimeScene.iStartFrame);
				pObj->m_fAnimInverse *= -1.0f;
			}

			//K_Matrix matWorld = pObj->m_AnimeTracks[pObj->m_fAnimeFrame].matAnime;
			K_Matrix matWorld = pObj->InterPlate(pObj->m_fAnimeFrame);
			pObj->SetMatrix(&matWorld, &m_pDebugCamera->m_matView, &m_pDebugCamera->m_matProj);
			pObj->Render();
		}
	}
