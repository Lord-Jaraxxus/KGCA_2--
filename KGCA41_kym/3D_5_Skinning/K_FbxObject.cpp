...
K_Matrix K_FbxObject::InterPlate(float fTime, K_AnimeScene AnimeScene)
{
	K_Matrix matIdentity;
	matIdentity.Identity();
	if (m_AnimeTracks.size() == 0) return matIdentity;
//   10                20
//   A=0 ------------ B=20
//   t=0  ~  t=0.5f ~  t=1
	K_AnimeTrack A, B;
	A = m_AnimeTracks[max(AnimeScene.iStartFrame, fTime + 0)];
	B = m_AnimeTracks[min(AnimeScene.iEndFrame, fTime + 1)];
