...
void K_FBXLoader::PreProcessS(FbxNode* pFbxNode)
{
	...
	pObject->m_pObjectName = pFbxNode->GetInitialName();
...
bool K_FBXLoader::UpdateFrameTest(ID3D11DeviceContext* pContext, K_AnimeScene pAnimeScene, float fTime)
{ 
	// object + skinning
	std::vector<K_Matrix> matCurrentAnimList;
	for (int iBone = 0; iBone < m_pObjectListS.size(); iBone++)
	{
		K_Matrix matAnimation = m_pObjectListS[iBone]->InterPlate(fTime, m_AnimeScene);
		m_cbDataBone.matBone[iBone] = matAnimation.Transpose();
		matCurrentAnimList.push_back(matAnimation);
	}
	pContext->UpdateSubresource(m_pConstantBufferBone, 0, nullptr, &m_cbDataBone, 0, 0);
		...
		if (m_pDrawObjListS[iDraw]->m_dxMatrixBindPseMap.size()) // skinning 있는놈만 이밑에걸 돌려라 같은디
		{
			for (int iBone = 0; iBone < m_pObjectListS.size(); iBone++)
			{
				...
				if (iter != m_pDrawObjListS[iDraw]->m_dxMatrixBindPseMap.end())
				{
					K_Matrix matBind = iter->second;
					K_Matrix matAnim = matBind * matCurrentAnimList[iBone];
					m_pDrawObjListS[iDraw]->m_cbDataBone.matBone[iBone] = matAnim.Transpose();
				}
			}
			pContext->UpdateSubresource(m_pDrawObjListS[iDraw]->m_pConstantBufferBone, 0, nullptr, &m_pDrawObjListS[iDraw]->m_cbDataBone, 0, 0);
		}
	}
...
void K_FBXLoader::UpdateSkeleton(ID3D11DeviceContext* pContext, float fTime)
{
	for (int iBone = 0; iBone < m_pObjectListS.size(); iBone++)
	{
		K_Matrix matAnimation = m_pObjectListS[iBone]->InterPlate(fTime, m_AnimeScene);
...
void K_FBXLoader::UpdateSkinning(ID3D11DeviceContext* pContext, VS_CONSTANT_BONE_BUFFER& cbInputData)
{	
	// skinning
	for (int iDraw = 0; iDraw < m_pDrawObjListS.size(); iDraw++)
	{
		if (m_pDrawObjListS[iDraw]->m_dxMatrixBindPseMap.size())
		{
			for (int iBone = 0; iBone < m_pObjectListS.size(); iBone++)
			{
				auto iter = m_pDrawObjListS[iDraw]->m_dxMatrixBindPseMap.find(iBone);
				...
				{
					K_Matrix matBind = iter->second;
					K_Matrix matAnim = cbInputData.matBone[iBone].Transpose();
					matAnim = matBind * matAnim;
					m_pDrawObjListS[iDraw]->m_cbDataBone.matBone[iBone] = matAnim.Transpose();
					pContext->UpdateSubresource(m_pDrawObjListS[iDraw]->m_pConstantBufferBone, 0, nullptr, &m_pDrawObjListS[iDraw]->m_cbDataBone, 0, 0);
				}
			}
