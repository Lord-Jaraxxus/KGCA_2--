...
bool K_Window::Run()
{
    if (Init() == false) // 이 Init은 밑에 샘플껏이 돌게될껐이다?
    {
       Release();
        return false;
    }
    ...
    while (m_bGameRun)
    {
        ...
        {
            if (!Frame() || !Render()) m_bGameRun = false;
        }
...
BOOL K_Window::InitInstance(const WCHAR* szTitle, UINT iWidth, UINT iHeight)
{
    ...
    ShowWindow(hWnd, SW_SHOW); // 윈도우가 화면에 뜬다. 쇼윈도에걸린너의셔츠를보며
    GetWindowRect(hWnd, &m_rtWindow); // 이거저거 다 있는 모든 화면영역
    GetClientRect(hWnd, &m_rtClient); // 이거저거 뺀 순수 출력화면
    g_rtClient = m_rtClient;
