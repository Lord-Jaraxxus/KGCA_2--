...
bool K_EliteBattleScene::Init()
{
	m_pBGM = I_Sound.Load(L"../../data/sound/audio/using/bgm/EliteBattle.ogg");
	...
	K_Sound* pSound16 = I_Sound.Load(L"../../data/sound/audio/using/ElliteBuff.ogg");
	m_pEffectSoundList.insert(std::make_pair(L"ElliteBuff", pSound16));
	K_Sound* pSound17 = I_Sound.Load(L"../../data/sound/audio/using/ElliteDefeat.ogg");
	m_pEffectSoundList.insert(std::make_pair(L"ElliteDefeat", pSound17));
	...
	// 적 캐릭터
	K_Texture* pEnemyTexture = I_Tex.Load(L"../../data/img/enemy/GremlinNob.png");
	K_Texture* pEnemyDefeatTexture = I_Tex.Load(L"../../data/img/enemy/GremlinNobDefeat.png");
	K_Texture* pEnemyHitTexture = I_Tex.Load(L"../../data/img/enemy/GremlinNobHit.png");
	...
	m_pEnemy->Create(m_pd3dDevice, m_pImmediateContext, L"../../data/img/enemy/GremlinNob.png", L"../KymGameCore/MaskShape_Png.txt");
	...
	K_Texture* pTextureEnemyIntent9 = I_Tex.Load(L"../../data/img/intent/buff1.png");
	K_Texture* pTextureEnemyIntent10 = I_Tex.Load(L"../../data/img/intent/attackDefend.png");
	K_Texture* pTextureEnemyIntent11 = I_Tex.Load(L"../../data/img/intent/defend.png");
	...
	m_pEnemyIntent->m_pEnemyIntentTextureList.push_back(pTextureEnemyIntent9);
	m_pEnemyIntent->m_pEnemyIntentTextureList.push_back(pTextureEnemyIntent10);
	m_pEnemyIntent->m_pEnemyIntentTextureList.push_back(pTextureEnemyIntent11);
...
bool K_EliteBattleScene::Render()
{
	...
	if (m_iEnemyIntentPower != 0) { I_Write.Draw(1320, 340, EnemyIntentPower, { 1,1,1,1 }); }
...
bool K_EliteBattleScene::Release()
{
...
void K_EliteBattleScene::SetDeck(K_Deck* deck)
{
	m_pDeck = deck;
}

void K_EliteBattleScene::SetPlayerHP(int currentHP, int maxHP)
{
	m_iPlayerCurrentHP = currentHP;
	m_iPlayerMaxHP = maxHP;
}

void K_EliteBattleScene::Combat(int turn)
{
	...
	if (turn % 2 == 0)
	{
		...
		if ((m_iTurn+1)/2 % 4 == 0 && m_bEnemyTurn == false)
		{
			m_iEnemyIntentPower = 0;
			m_pEnemyIntent->m_pTexture = m_pEnemyIntent->m_pEnemyIntentTextureList[8];
			m_pEnemyIntent->SetMask(m_pEnemyIntent->m_pEnemyIntentTextureList[8]);
		}
		else if ((m_iTurn + 1)/2 % 4 == 1 && m_bEnemyTurn == false)
		{
			m_iEnemyIntentPower = 30;
			m_pEnemyIntent->m_pTexture = m_pEnemyIntent->m_pEnemyIntentTextureList[6];
			m_pEnemyIntent->SetMask(m_pEnemyIntent->m_pEnemyIntentTextureList[6]);
		}
		else if ((m_iTurn + 1)/2 % 4 == 2 && m_bEnemyTurn == false)
		{
			m_iEnemyIntentPower = 12;
		...
		else if ((m_iTurn + 1)/2 % 4 == 3 && m_bEnemyTurn == false)
		{
			m_iEnemyIntentPower = 14;
			m_pEnemyIntent->m_pTexture = m_pEnemyIntent->m_pEnemyIntentTextureList[4];
			m_pEnemyIntent->SetMask(m_pEnemyIntent->m_pEnemyIntentTextureList[4]);
		}
	}
	else
	{
...
void K_EliteBattleScene::CardUsed(int card_num)
{
	switch (card_num)
	{
	case Strike:
...
void K_EliteBattleScene::EnemyTurn(int turn)
{
	m_iTurn++;
	...
	{
	case 1:
		m_pEffectSoundList.find(L"ElliteBuff")->second->PlayEffect();
		break;
...
void K_EliteBattleScene::CardSetting()
{
	for (int i = 0; i < m_pCardList.size(); i++)
	{
...
void K_EliteBattleScene::EffectProcessing()
{
...
void K_EliteBattleScene::EnemyTurnProcessing()
{
...
void K_EliteBattleScene::CharacterActionProcessing()
{
	// 플레이어
...
void K_EliteBattleScene::CameraShake()
{
...
void K_EliteBattleScene::EnemyDefeat()
{
	m_fEnemyTurnTimer += g_fSecondPerFrame;
