...
bool Sample::Init() 
{
	...
	m_Sound.Init();
	m_Sound.Load(L"../../data/sound/월피스카터  - 이별만이 인생이다.mp3");
...
bool Sample::Frame()
{
	...
	if (I_Input.GetKey(VK_F1) == KEY_PUSH) m_Sound.Play(1);
	if (I_Input.GetKey(VK_F2) == KEY_PUSH) m_Sound.Stop();
	if (m_Sound.m_pChannel != nullptr) 
	{
		if (I_Input.GetKey(VK_RBUTTON) == KEY_PUSH) m_Sound.Pause();
		if (I_Input.GetKey(VK_F3) == KEY_PUSH) m_Sound.Volume(true);
		if (I_Input.GetKey(VK_F4) == KEY_PUSH) m_Sound.Volume(false);
	}
	m_Sound.Frame();
...
bool Sample::Render()
{
	...
	I_Write.Draw(0, 100, m_Sound.m_szBuffer);
