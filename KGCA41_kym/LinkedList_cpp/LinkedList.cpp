...
k_Student* LinkedList::begin() // head 바로 다음 노드를 리턴
{
	k_Student* temp = g_pHead->m_pNext;
	return temp;
}

k_Student* LinkedList::end() // Tail 바로 앞 노드를 리턴
{
...
k_Student* LinkedList::find_young_man(k_Student* temp)
{
...
k_Student* LinkedList::find(int num)
{
	k_Student* temp = g_pHead->m_pNext;
	while (temp != g_pTail)
	{
